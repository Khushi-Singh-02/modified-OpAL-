...
#this code is set to recover ag, an from the model. However it can be modified to any 
#2 parameters of choice. Simply replace ag, an with desired parameters to recover in the code 
#ahead. Values for ac, bg, bn are commented to facilitate this.
...
for i in range(2):
    ...
    plt.tight_layout()
    plt.savefig(f'OpAL_2p_fit_plot_{para[i]}_recovery')
    plt.close()
...
results.to_csv('Model_fitting_results_OpAL_2p.csv', index=False)
