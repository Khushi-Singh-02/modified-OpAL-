...
# list all instnaces of each combination (ag, an) in the simulated data
def find_all_occurrences(ag, an):
    indexes = [] 
    with open(data_file, 'r') as file:
        ...
        for index, row in enumerate(csv_reader, start=1):
            if row['Alpha_a_GO'] == str(ag) and row['Alpha_a_NOGO'] == str(an):
...
for i in range(len(num_combos)): 
    ...
    indexes = find_all_occurrences(str(ag), str(an)) 
    df = pd.read_csv(data_file, usecols=['Choice', 'Outcome'])
    rat_choice = df['Choice'].iloc[indexes].astype(int).to_numpy() - 1 
    rat_reward = df['Outcome'].iloc[indexes].to_numpy() 
...
for i in range(2):
    ...
    plt.tight_layout()
    plt.savefig(f'OpAL_LR_fit_plot_{para[i]}_recovery')
    plt.close()
...
results.to_csv('Model_fitting_results_OpAL_LR.csv', index=False)
