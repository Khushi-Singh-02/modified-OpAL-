...
def ll_func(params, rat_choice, rat_reward, v0=0.0, crit="SA", mod = "avg_value", k=1 , norm=False, mag=1, hebb=True, variant = None, decay_to_prior = True, decay_to_prior_gamma = 1):
	...
	alpha_a_Go, alpha_a_NoGo, alpha_c, beta_GO, beta_NOGO = params
	ll=0
...
for i in range(5):
    ...
    plt.tight_layout()
    plt.savefig(f'OpAL_5p_fit_plot_{para[i]}_recovery')
    plt.close()
...
results.to_csv('Model_fitting_results_OpAL_5p.csv', index=False)
